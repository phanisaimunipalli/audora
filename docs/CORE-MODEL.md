...
| **Plan reader** — **v0 shipped (zero-shot)** | today MiniCPM-V-4.5 zero-shot; next Qwen2.5-VL 7B LoRA | floor plan image → rooms, dimensions, north arrow, storeys | 11 plans / 77 labelled rooms in `evals/plans` (8 synthetic sheets with exact ground truth, the demo townhouse, two 1911 Guimard plans as hard cases); unlimited synthetic plans from `evals/plans/make-plans.mjs` | room recall 86 %, type 100 %, 91 % of returned dimensions within 5 %, ~4 s, $0.001/plan (zero-shot) | misses are missing rooms and hallucinated extras (23 across the corpus) and north-arrow detection (40 %); synthetic data covers the long tail cheaply for the fine-tune |
