/**
 * The Marble prompt compiler — docs/BACKEND.md section 3. One copy, imported by both sides.
 *
 ...
 * recipe (server/recipe.ts, and `browserRecipe` in src/services/marble.ts) and is hashed, which is
 * why this module is a pure function of its input: no clock, no randomness, no locale-dependent
 * formatting, and nothing guessed — an unknown fact is left out rather than filled in.
 *
 * This file used to exist twice, byte-identical, in `server/prompt.ts` and
 * `src/services/marblePrompt.ts`, kept in step by a parity test. It now lives in `shared/`, which
 * both builds compile, and those two modules re-export it: a room generated from the wizard and the
 * same room generated by the backend describe themselves in the same words because it is the same
 * function, not because a test says so.
 *
 * Conventions:
 ...
 * - Types that mirror src/ are declared here, not imported: `shared/` depends on nothing.
