  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-6 px-4 py-10 md:px-6">
      ...
      <Card className="flex flex-col gap-4">
        ...
        <Callout tone={providers.marble ? 'ok' : 'warn'} title={providers.marble ? 'World Labs Marble is live' : 'World Labs Marble is mocked'}>
          {providers.marble
            ? `Generations call the real API with ${providers.models?.marbleDraft ?? 'marble-1.0-draft'} (draft) and ${providers.models?.marbleFull ?? 'marble-1.1'} (full).`
        ...
        <Callout tone={providers.nebius ? 'ok' : 'warn'} title={providers.nebius ? 'Nebius Token Factory is live' : 'Nebius Token Factory is mocked'}>
          {providers.nebius
      ...
      <Card className="flex flex-col gap-4">
        <div className="text-sm font-medium">Reconstruction</div>
        ...
        {providers.marble ? <p className="mono text-xs text-ink-3">live generations this server session: {providers.liveGenerations ?? 0} / {providers.maxGenerations ?? 3} (raise MARBLE_MAX_GENERATIONS in .env)</p> : null}
      </Card>
      <Card className="flex flex-col gap-4">
        <div className="text-sm font-medium">Simulated generation timing (mock mode)</div>
      ...
      <Card className="flex flex-col gap-4">
        ...
        <p className="text-sm text-ink-2">Generation takes minutes. Allow notifications and you can leave the tab; we will tell you when the tour is ready.</p>
        {perm === 'granted' ? (
          <Chip tone="ok" mono className="self-start">browser notifications on</Chip>
        ) : perm === 'unsupported' ? (
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="mono text-xs text-ink-3">{perm === 'denied' ? 'blocked in the browser settings' : 'not asked yet'}</span>
          </div>
        )}
      </Card>
      <Card className="flex flex-col gap-4">
        <div className="text-sm font-medium">Agent identity</div>
        <div className="grid grid-cols-2 gap-4">
          <Field label="Name"><Input value={settings.agentName} onChange={(e) => set({ agentName: e.target.value })} /></Field>
          <Field label="Brand colour">
            <label className="flex h-10 items-center gap-3 rounded-xl border border-line-2 bg-bg-2 px-3">
      ...
      <Card className="flex flex-col gap-3">
        ...
        <p className="text-sm text-ink-2">Everything lives in this browser's storage. Reset clears your tours and re-seeds the demo listing.</p>
        <div>
