...
export function FloorPlanSvg({
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className={cx('block h-auto w-full', className)} role="img" aria-label={`Floor plan, ${g.width.toFixed(2)} by ${g.depth.toFixed(2)} metres`}>
      <rect x={px(-g.width / 2)} y={pz(-g.depth / 2)} width={g.width * S} height={g.depth * S} fill="var(--color-bg)" stroke="var(--color-ink)" strokeWidth={wallW} />
      {g.windows.map((wn, i) => {
      ...
      <line x1={px(dA.x)} y1={pz(dA.z)} x2={px(dB.x)} y2={pz(dB.z)} stroke="var(--color-bg)" strokeWidth={wallW + 2} />
      <path
      ...
      {[...flats, ...solids].map((p) => {
        ...
        return (
          <g key={p.id}>
            <polygon points={pts} fill={fill} fillOpacity={p.flat ? 0.3 : 0.85} stroke={buyer ? 'var(--color-buyer)' : 'rgba(10,10,10,0.35)'} strokeWidth={2} />
            {showNames && !p.flat ? (
              <text x={px(p.x)} y={pz(p.z)} textAnchor="middle" dominantBaseline="middle" fontSize={14} fill="var(--color-ink)" fontFamily="var(--font-mono)">
      ...
      {showDims ? (
        <g fontFamily="var(--font-mono)" fontSize={26} fill="var(--color-dim)">
